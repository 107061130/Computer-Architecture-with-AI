...
object OutputSel extends ChiselEnum {
  val ADDSUB, MUL, QUAN, REG, OTHERS = Value
}
...
class SIMDEngine extends Module {
  ...
  val quanUnit             = Module(new QuanUnit())
  val register             = Module(new Register())
  val others               = Module(new OthersUnit())
  ...
  mulUnit.io.scale              := quanUnit.io.scale
  mulUnit.io.zero               := quanUnit.io.zero

  quanUnit.io.opSel             := controller.io.quanOpSel
  quanUnit.io.rs1               := io.cmd_payload.bits.rs1.asUInt
  quanUnit.io.rs2               := io.cmd_payload.bits.rs2.asUInt
  ...
  others.io.opSel               := controller.io.othersOpSel
  others.io.rs1                 := io.cmd_payload.bits.rs1.asUInt
  ...
  io.rsp_payload.bits.rd := MuxLookup(
    ...
    Seq(
      ...
      OutputSel.QUAN.asUInt   -> quanUnit.io.rd.asSInt,
      OutputSel.REG.asUInt    -> register.io.rdMsbOut.asSInt,
      OutputSel.OTHERS.asUInt -> others.io.rd.asSInt
    )
  )
}
